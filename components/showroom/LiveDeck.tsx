  ...
  return <div className="live-deck-wrap">
    ...
    <motion.div className={`live-deck preview-${demo.slug}`} onPointerMove={track} onPointerLeave={() => { mx.set(0); my.set(0); }} style={reduceMotion ? {} : { rotateX, rotateY, transformPerspective: 1200 }}>
      <AnimatePresence initial={false} mode="wait">
        <motion.div className={`deck-scene preview-${demo.slug}`} key={demo.slug} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduceMotion ? 0 : .18 }}>
          <Image src={demo.image} alt={demo.imageAlt} fill sizes="(max-width: 760px) 100vw, 58vw" preload={active === 0} className="deck-photo" />
