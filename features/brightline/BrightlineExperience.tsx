...
export function BrightlineExperience(){
 ...
 const zipValid=/^\d{5}$/.test(zip);const zipTouched=zip.length>0;
 return <main className="bright-shell" id="main-content">
   ...
   <section className="bright-flow"><aside><span>LIVE INTAKE</span><h2>One minute<br/>to a useful lead.</h2><p>No generic “contact us” form. Every answer gives the service team context.</p></aside><div className="bright-intake-card"><div className="intake-top"><span>ACTIVE REQUEST</span><b>{service.name}</b></div><label><span>ZIP code</span><input value={zip} onChange={e=>setZip(e.target.value.replace(/\D/g,"").slice(0,5))} inputMode="numeric" pattern="\d{5}" maxLength={5} required aria-invalid={zipTouched&&!zipValid} aria-describedby="bright-zip-hint" /><small id="bright-zip-hint" className={zipTouched&&!zipValid?"field-error":"field-hint"}>{zipTouched&&!zipValid?"Enter a valid 5-digit ZIP code.":"5-digit U.S. ZIP code"}</small></label><div className="urgent-row"><div><span>Urgency</span><strong>{urgent?"Active issue":"Can schedule"}</strong></div><button type="button" className={urgent?"active":""} onClick={()=>setUrgent(v=>!v)}><i></i>{urgent?"Urgent":"Routine"}</button></div><div className="window-row"><span>Preferred window</span><div>{windows.map(w=><button type="button" key={w} className={window===w?"active":""} onClick={()=>setWindow(w)}>{w}</button>)}</div></div><div className="dispatch-total"><div><span>Dispatch estimate</span><small>applied to completed work</small></div><strong>${dispatch}</strong></div><button className="intake-continue" type="button" disabled={!zipValid} onClick={()=>{setDone(false);setOpen(true)}}>Continue request <ArrowIcon/></button>{!zipValid&&<small className="field-error intake-continue-note">Add a 5-digit ZIP code to continue.</small>}</div></section>
   ...
   <AnimatePresence>{open&&<motion.div className="bright-layer" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><button className="bright-scrim" type="button" aria-label="Close request" onClick={()=>setOpen(false)}/><motion.aside className="bright-panel" initial={{y:"100%"}} animate={{y:0}} exit={{y:"100%"}} transition={{duration:.5,ease:[.2,.75,.18,1]}}><header><div><span>DEMO SERVICE REQUEST</span><h2>{done?"Dispatch ready.":"Confirm the visit."}</h2></div><button type="button" onClick={()=>setOpen(false)}>Close</button></header>{!done?<div className="bright-panel-body"><div className="request-spec"><span>System</span><b>{service.name}</b></div><div className="request-spec"><span>Priority</span><b>{urgent?"Urgent":"Routine"}</b></div><div className="request-spec"><span>ZIP</span><b className={!zipValid?"field-error":""}>{zip||"—"}</b></div><div className="request-spec"><span>Window</span><b>{window}</b></div><div className="request-price"><span>Dispatch estimate</span><strong>${dispatch}</strong></div><button type="button" disabled={!zipValid} onClick={()=>setDone(true)}>Request this visit <ArrowIcon/></button><small className={!zipValid?"field-error":""}>{!zipValid?"Enter a valid 5-digit ZIP code above to continue.":"Demo mode — nothing is dispatched or charged."}</small></div>:<div className="bright-success"><div className="success-pulse"><i></i></div><span>REQUEST / READY</span><h3>{service.name}<br/>{window}</h3><p>A production build would send this to the business and trigger customer confirmation.</p><button type="button" onClick={()=>setOpen(false)}>Return home</button></div>}</motion.aside></motion.div>}</AnimatePresence>
