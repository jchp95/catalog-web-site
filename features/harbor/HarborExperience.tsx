...
export function HarborExperience() {
  ...
  return <main className="harbor-shell" id="main-content">
    ...
    {active && <section id="property" className="harbor-selected"><div className="harbor-selected-visual"><Image key={active.image} src={active.image} alt={`Illustrative exterior of ${active.name}`} fill sizes="(max-width: 900px) 100vw, 60vw" /><small>{active.area} / SAMPLE PROPERTY</small></div><div className="harbor-selected-copy"><span>A CLOSER LOOK / {active.area}</span><h2>{active.name}</h2><p>{active.beds} beds · {active.baths} baths · {active.sqft.toLocaleString("en-US")} sqft</p><strong>{fmt(active.price)}</strong><hr /><p>{active.description}</p><button type="button" onClick={openTour}>Picture yourself here <ArrowIcon /></button><small>Explore a private tour request in demo mode.</small></div></section>}
